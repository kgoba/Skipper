...
public class WeatherProvider {
    ...
    public WeatherForecast doStuff() {
    ...
    private void parseJson(JsonReader reader, WeatherForecast forecast) throws IOException {
        reader.beginObject();
        while (reader.hasNext()) {
            String name = reader.nextName();
            if (name.equals("list")) {
                parseJsonList(reader, forecast);
            }
            else {
                reader.skipValue();
            }
    ...
    private void parseJsonList(JsonReader reader, WeatherForecast forecast) throws IOException {
        reader.beginArray();
        while (reader.hasNext()) {
            WeatherForecast.Entry entry = forecast.newEntry();
            reader.beginObject();
            while (reader.hasNext()) {
                String name = reader.nextName();
                if (name.equals("dt")) {
                    entry.dt = reader.nextInt();
                }
                else if (name.equals("wind")) {
                    parseJsonWind(reader, entry);
                }
                else {
                    reader.skipValue();
                }
    ...
    private void parseJsonWind(JsonReader reader, WeatherForecast.Entry entry) throws IOException {
        reader.beginObject();
        while (reader.hasNext()) {
            String name = reader.nextName();
            if (name.equals("speed")) {
                entry.windSpeed = (float)reader.nextDouble();
            }
            else if (name.equals("deg")) {
                entry.windDirection = (float)reader.nextDouble();
            }
            else {
                reader.skipValue();
            }
